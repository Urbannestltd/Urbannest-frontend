...
export const ExpenseLogging = ({ id }: { id: string }) => {
    ...
    return <Box px={{ base: 4, md: 8 }}>
        <SectionFlex w={'full'} justify={{ base: 'flex-start', md: 'center' }} bg={'#F0F4F780'}>
            {isMobile ?
                <Box w={'full'}>
                    <HStack w={'full'} justify={'space-between'}>
                        ...
                        <Drawers placement={{ base: 'bottom', md: 'end' }} open={open} onOpenChange={setOpen} triggerElement={<Center bg={'#CAD5ED4D'} p={2} rounded={'sm'}><MdOutlineAccountBalanceWallet color="#545F73" /> </Center>} modalContent={<LogExpense ticketid={id} onClose={() => setOpen(false)} />} />
                    </HStack>
            ...
            {!isMobile && <Drawers className="min-w-[450px] w-fit" placement={{ base: 'bottom', md: 'end' }} open={open} onOpenChange={setOpen} triggerElement={<MainButton size='lg' icon={<BiPlusCircle />} className="h-[40px]">Log Expense</MainButton>} modalContent={<LogExpense ticketid={id} onClose={() => setOpen(false)} />} />
            }</SectionFlex>
        {isMobile ?
            <MobileTable tableName="Expense Log" data={expenses} /> : <DataTable
                columns={columns}
                borderedHeader
                headerColor="#ffffff"
                data={expenses}
