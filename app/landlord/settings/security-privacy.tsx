...
import { ChangePassword, ChangePasswordPayload, getTwoFactorStatus, updateTwoFactorStatus } from "@/services/landlord/settings"
...
export const SecurityPrivacy = () => {
    ...
    useEffect(() => {
        const fetchTwofa = async () => {
            try {
                ...
                setTwofa(response.isTwoFactorEnabled)
            } catch {
                setTwofa(false)
            }
        }
    ...
    const handle2fa = async (twofa: boolean) => {
        if (twofa) {
            try {
                const payload = { enabled: twofa }
                await updateTwoFactorStatus(payload);
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
                <HStack justify={'space-between'}>
                    ...
                    <Flex gap={2}>
                        ...
                        <MainButton className="h-[35px]" type='submit' loading={mutate.isPending} disabled={mutate.isPending || !formState.isValid || !formState.isDirty} size='sm'>Save</MainButton>
                    </Flex>
                </HStack>
                <Divider my={10} />
