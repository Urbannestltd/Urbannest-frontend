...
import { UpdateUserProfile, UserProfilePayload } from "@/services/landlord/settings"
...
export const AccountInfo = () => {
    ...
    const { userSettings, fetchUserSettings } = useLandlordSettingStore((state) => state)
    ...
    const onSubmit = async (data: PersonalInfoFormData) => {
        const profileUrl = file
            ? await StoreFile({ folder: 'support', file })
            : userSettings?.userProfileUrl
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
                <HStack justify={'space-between'}>
                    ...
                    <Flex gap={2}>
                        ...
                        <MainButton type="submit" loading={mutate.isPending} className="h-[35px]" size='sm'>Save</MainButton>
                    </Flex>
                </HStack>
                <Divider my={10} />
