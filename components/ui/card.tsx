        ...
        <Grid gap={4} templateColumns={{ base: newMobile ? "repeat(2, 1fr)" : "repeat(1, 1fr)", md: "repeat(4, 1fr)" }} w={{ base: "auto", md: "full" }} minH={newMobile ? "180px" : "118px"} h={newMobile ? 'auto' : 'fit'} maxH={{ base: "auto", md: "400px" }}>
            {data.map((item, index) => (
                <Flex
                    ...
                    minW={'184px'}
                    bg={'white'}
                    h={{ base: "full", md: item.icon ? "155px" : "full" }} wrap={'wrap'}
                    rounded={item.icon ? "12px" : "8px"}
                    border={item.border ? `1px solid #E0E0E0` : "1px solid "}
                ...
                >
                    {isMobile ? <HStack>
                        ...
                        <Flex ml={1} direction={'column'} w={'full'}>
                            <Text fontSize={'14px'} color={"#5A5A5A"} className={item.isTitleBold ? "satoshi-bold" : "satoshi-medium"}>{item.title}</Text>
                            <Flex align={'end'} gap={1}>
                                <Text color={item.titleColor ?? '#303030'} fontSize={{ base: "20px", md: "32px" }} lineHeight={'32px'} className="font-semibold">
                        ...
                        {(item.icon || item.new) && <Flex mb={2} justify={'space-between'}>
                            {item.icon && <Image src={item.icon.src} alt={item.title} />}
                            {item.new && <Center h={'29px'} rounded={'7px'} fontSize={'12px'} px={'9px'} py={'4.5px'} bg={item.cardColor} color={item.newColor} className="satoshi-bold">+{item.new} NEW</Center>}
                        </Flex>}
